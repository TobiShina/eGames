...
function renderLiveBroadcast(players) {
  ...
  const streamablePlayers = players
    .filter((p) => p.twitchUrl)
...
async function renderComments(matchId) {
  const commentsContainer = document.querySelector(
    `.comment-content[data-match-id="${matchId}"]`,
  );
  const toggleButton = document.querySelector(
    `.comment-section-toggle[data-match-id="${matchId}"]`,
  );
      ...
      if (snapshot.empty) {
        ...
        snapshot.forEach((doc) => {
          ...
          const isLiked =
            currentLoggedInUser &&
            comment.likes &&
            comment.likes.includes(currentLoggedInUser.uid);
          ...
          commentItem
            ...
            .addEventListener("click", () => toggleLike(matchId, doc.id));
        });
      }
  ...
  if (!commentForm.dataset.listenerAttached) {
    postBtn.addEventListener("click", () =>
      postComment(matchId, commentInput, postBtn),
    );
  ...
  try {
    await db
      ...
      .add({
        ...
        authorName:
          currentLoggedInUser.displayName || currentLoggedInUser.email,
...
async function renderLiveScores() {
  ...
  const endOfDay = firebase.firestore.Timestamp.fromDate(
    new Date(filterDateStr + "T23:59:59"),
  );
  ...
  try {
    ...
    snapshot.forEach((doc) => {
      ...
      if (!groupedMatches[country][tournament])
        groupedMatches[country][tournament] = [];
        ...
        groupedMatches[country][tournament].forEach((match) => {
          const statusText =
            match.status === "upcoming"
              ? formatDateTime(match.date).split(", ")[1]
              : match.status.toUpperCase();
          const score =
            match.score1 !== null ? `${match.score1} - ${match.score2}` : "vs";
          const liveIndicator = match.isLive
            ? `<div class="live-indicator blinking"></div>`
            : `<div class="live-indicator" style="background-color: transparent;"></div>`;
...
// Add this if you want to perform any dynamic actions on load
// document.addEventListener("DOMContentLoaded", () => {
//    console.log("Services banner initialized.");
// });
